...
def load_file_in_root(file_name, path_list=[], root=""):
    file_path = get_root_path(file_name, path_list, root)
    with open(file_path, 'rb') as f:
        obj = pickle.load(f)
    return obj


def dump_file_in_root(file_name, content, path_list=[], root=""):
    file_path = get_root_path(file_name, path_list, root, create_if_not_exist=True)
    with open(file_path, 'wb') as f:
        pickle.dump(content, f)


def load_model_in_root(file_name, path_list=[], root=""):
    file_path = get_root_path(file_name, path_list, root)
    with open(file_path, 'rb') as f:
        model = torch.load(f)
    return model
...
def dump_model_in_root(file_name, model, path_list=[], root=""):
    file_path = get_root_path(file_name, path_list, root, create_if_not_exist=True)
    with open(file_path, 'wb') as f:
        torch.save(model, f)


def isfile_in_root(file_name, path_list=[], root=""):
    file_path = get_root_path(file_name, path_list)
    return os.path.isfile(file_path)
...
def get_root_path(file_name, path_list, root, create_if_not_exist=False):
    file_dir = os.path.join(root, *path_list)
    file_path = os.path.join(file_dir, file_name)
    if create_if_not_exist and not os.path.isdir(file_dir):
        os.makedirs(file_dir)
    return file_path
...
def file_exist(file_name, path_list, root=''):
    file_dir = os.path.join(root, *path_list)
    file_path = os.path.join(file_dir, file_name)
